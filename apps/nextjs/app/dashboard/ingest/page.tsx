  ...
  return (
    <div style={{
      ...
      {result.ingested !== undefined && (
        <Stat label="Ingested" value={result.ingested} color="#22c55e" />
      )}
      {result.skipped_duplicates !== undefined && (
        <Stat label="Duplicates skipped" value={result.skipped_duplicates} color="#555" />
      )}
      {result.errors !== undefined && (
        <Stat label="Errors" value={result.errors} color={result.errors > 0 ? '#ef4444' : '#555'} />
      )}
      {result.sources_polled !== undefined && (
        <Stat label="Sources polled" value={result.sources_polled} color="#F5A623" />
      )}
    </div>
  );
}
...
export default function IngestPage() {
  ...
  const [pollResult, setPollResult] = useState<Result | null>(null);
    ...
    <div>
      <div style={{ marginBottom: '32px' }}>
        ...
        <p style={{ margin: 0, color: '#666', fontSize: '14px' }}>
          Pull news signals into the platform. All items pass through full scoring, deduplication, and clustering.
        </p>
      </div>
      ...
      <Section title="RSS — Poll All Active Feeds">
        <p style={{ margin: '0 0 14px', fontSize: '13px', color: '#555' }}>
          Fetches the current feed for every active RSS source. Most RSS feeds carry 2–8 weeks of items depending on publication volume.
        </p>
        <button
          onClick={handlePoll}
          disabled={polling}
          style={{
            padding: '9px 18px',
            background: polling ? '#1a1a1a' : '#1a1a1a',
            border: '1px solid #333',
            color: polling ? '#555' : '#ccc',
            borderRadius: '4px',
            fontSize: '13px',
            cursor: polling ? 'not-allowed' : 'pointer',
          }}
        >
          {polling ? 'Polling feeds…' : 'Poll RSS Feeds Now'}
        </button>
        {polling && (
          <p style={{ marginTop: '10px', fontSize: '12px', color: '#555' }}>
            This may take a minute if you have many sources…
          </p>
        )}
        {pollResult && <ResultBadge result={pollResult} />}
      </Section>

      {/* HN Backfill */}
      <Section title="Hacker News — Historical Backfill">
        <p style={{ margin: '0 0 14px', fontSize: '13px', color: '#555' }}>
          Uses the Algolia HN Search API (free, no auth). Supports full history going back years.
          Tip: leave query blank to pull all top stories; add keywords to focus on a topic.
        </p>
        <form onSubmit={handleHN}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '12px' }}>
            <div>
              ...
              <input
                ...
                placeholder="e.g. AI agents, LLM, vibe coding"
                style={inputStyle}
              />
            </div>
            <div>
            ...
            <div>
              ...
              <input
                ...
                value={hnDays}
                onChange={e => setHnDays(e.target.value)}
                style={inputStyle}
              />
            ...
            <div>
              ...
              <input
                ...
                value={hnMax}
                onChange={e => setHnMax(e.target.value)}
                style={inputStyle}
              />
            </div>
          </div>
          ...
          <button
            ...
            style={{
              ...
              fontSize: '13px',
              fontWeight: 600,
              cursor: hnRunning ? 'not-allowed' : 'pointer',
            }}
          >
          ...
          {hnRunning && (
            <p style={{ marginTop: '10px', fontSize: '12px', color: '#555' }}>
              Processing — large backfills (365 days, 500+ items) may take several minutes while signals are embedded and scored…
            </p>
          )}
      ...
      <Section title="Reddit — Historical Backfill">
        <p style={{ margin: '0 0 14px', fontSize: '13px', color: '#555' }}>
          Pulls top posts from a subreddit via the public Reddit API. Note: Reddit enforces rate limits (~1 req/sec) so large backfills take a few minutes.
        </p>
        <form onSubmit={handleReddit}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '12px' }}>
            ...
            <div>
              ...
              <input
                ...
                value={redditDays}
                onChange={e => setRedditDays(e.target.value)}
                style={inputStyle}
              />
            </div>
            <div>
              ...
              <input
                ...
                value={redditMax}
                onChange={e => setRedditMax(e.target.value)}
                style={inputStyle}
              />
            </div>
          </div>
          ...
          <button
            ...
            style={{
              ...
              fontSize: '13px',
              fontWeight: 600,
              cursor: redditRunning ? 'not-allowed' : 'pointer',
            }}
          >
      ...
      {/* Notes */}
      <div style={{ marginTop: '8px', padding: '16px', background: '#111', border: '1px solid #1e1e1e', borderRadius: '4px' }}>
